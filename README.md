...
**④ 同時実行制御による二重貸出（重複貸出）の防止**
・問題点: 複数のユーザーがほぼ同時に同じ備品に対して貸出申請を行った際、処理のタイミングによってデータの不整合（Data Inconsistency）が生じ、一つの備品が二人のユーザーに同時に貸し出される「二重貸出」のリスクを確認。
・解決策: JPAの Pessimistic Lock（悲観的ロック） を導入。特定の備品の貸出プロセスが開始される際、該当するDBレコードを占有（Lock）することで、他のトランザクションによる同時修正やアクセスを制御。
・効果: 高負荷時や同時アクセスが多い環境下でも、備品の在庫数とステータスを正確に管理でき、データの整合性（Data Integrity）を完全に保証。
